    ...
    return {
        ...
        submit: function(q) {
            if (!m) {
                ...
                $.post("/register", $("#register-new-user").serialize()).done(c).fail(a).always(e)
            }
            q.preventDefault();
            return false
        },
...
var EuCookiePolicy = (function() {
    return {
        init: function() {
            var c = Cookie.get("habbocpolicy");
            if (c == null) {
                var a = jQuery("#eu_cookie_policy");
                a.show();
                b(1);
                jQuery("#eu_cookie_policy_close").click(function() {
                    Cookie.set("habbocpolicy", "hidden", 730);
                    b(-1);
                    jQuery("#eu_cookie_policy").hide()
                })
            }

            function b(d) {
                var e = jQuery("header");
                if (e.hasClass("login-error")) {
                    e.attr("style", "height: " + (e.height() + a.height() * d) + "px !important")
                }
            }
        }
    }
